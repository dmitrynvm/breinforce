...
class Judge(object):
    ...
    def __init__(
        ...
        self.hashmap = LookupTable(
            suits, ranks, cards_for_hand, low_end_straight=low_end_straight, order=order
        )

        total = sum(
            self.hashmap.hand_dict[hand]["suited"] for hand in self.hashmap.ranked_hands
        )

        hands = [
            "{} ({:.4%})".format(hand, self.hashmap.hand_dict[hand]["suited"] / total)
    ...
    def evaluate(
        ...
        for card_comb in all_card_combs:
            score = self.hashmap.lookup(list(card_comb))
            if score < minimum:
                minimum = score
        return minimum
    ...
    def get_rank_class(self, hand_rank: int) -> str:
        ...
        for hand in self.hashmap.ranked_hands:
            if hand_rank <= self.hashmap.hand_dict[hand]["cumulative unsuited"]:
                return hand
        raise errors.InvalidHandRankError(
            (
                f"invalid hand rank, expected 0 <= hand_rank"
